
import React from 'react';
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectGroup, SelectItem, SelectLabel, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Checkbox } from "@/components/ui/checkbox";
import { Button } from "@/components/ui/button";
import { Plus } from "lucide-react";
import { UniformSize, Player } from "@/types";

const SIZES = {
  adult: ['S', 'M', 'L', 'XL', '2XL', '3XL', '4XL'] as const,
  kids: ['1', '3', '5', '7', '9', '11', '13', '15'] as const
};

interface PlayerFormFieldsProps {
  player: Player;
  onPlayerChange: (updates: Partial<Player>) => void;
  onSubmit: () => void;
  onCancel: () => void;
  isEditing: boolean;
  printStyleOptions: string[];
  isMobile?: boolean;
}

const PlayerFormFields = React.memo(({
  player,
  onPlayerChange,
  onSubmit,
  onCancel,
  isEditing,
  printStyleOptions,
  isMobile
}: PlayerFormFieldsProps) => {
  return (
    <div className={isMobile ? "p-4" : ""}>
      <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
        <div className="md:col-span-2">
          <Label htmlFor="playerNumber">Số áo</Label>
          <Input
            id="playerNumber"
            type="text"
            value={player.number || ""}
            onChange={e => onPlayerChange({ number: e.target.value })}
            placeholder="Số áo"
          />
        </div>
        
        <div className="md:col-span-2">
          <Label htmlFor="line1">Tên trên số</Label>
          <Input
            id="line1"
            value={player.line_1 || ""}
            onChange={e => onPlayerChange({ line_1: e.target.value })}
            placeholder="Tên trên số lưng"
          />
        </div>
        
        <div className="md:col-span-2">
          <Label htmlFor="line3">Tên dưới số</Label>
          <Input
            id="line3"
            value={player.line_3 || ""}
            onChange={e => onPlayerChange({ line_3: e.target.value })}
            placeholder="Tên đội bóng"
          />
        </div>

        <div>
          <Label htmlFor="playerSize">Size</Label>
          <Select
            value={player.size}
            onValueChange={value => onPlayerChange({ size: value as UniformSize })}
          >
            <SelectTrigger id="playerSize">
              <SelectValue placeholder="Size" />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectLabel>Người lớn</SelectLabel>
                {SIZES.adult.map(size => (
                  <SelectItem key={size} value={size}>{size}</SelectItem>
                ))}
              </SelectGroup>
              <SelectGroup>
                <SelectLabel>Trẻ em</SelectLabel>
                {SIZES.kids.map(size => (
                  <SelectItem key={size} value={size}>{size}</SelectItem>
                ))}
              </SelectGroup>
            </SelectContent>
          </Select>
        </div>

        <div className="md:col-span-2">
          <Label htmlFor="uniformType">Loại quần áo</Label>
          <Select
            value={player.uniform_type || "player"}
            onValueChange={value => onPlayerChange({ uniform_type: value as 'player' | 'goalkeeper' })}
          >
            <SelectTrigger id="uniformType">
              <SelectValue placeholder="Loại" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="player">Cầu thủ</SelectItem>
              <SelectItem value="goalkeeper">Thủ môn</SelectItem>
            </SelectContent>
          </Select>
        </div>

        <div className="md:col-span-2">
          <Label htmlFor="printStyle">Kiểu in</Label>
          <Select
            value={player.print_style || printStyleOptions[0]}
            onValueChange={value => onPlayerChange({ print_style: value })}
          >
            <SelectTrigger id="printStyle">
              <SelectValue placeholder="Chọn kiểu in" />
            </SelectTrigger>
            <SelectContent>
              {printStyleOptions.map(style => (
                <SelectItem key={style} value={style}>{style}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>

        <div className="md:col-span-4 space-y-4">
          <div>
            <Label className="mb-2 inline-block">In số</Label>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="chestNumber"
                  checked={player.chest_number || false}
                  onCheckedChange={checked => onPlayerChange({ chest_number: checked === true })}
                />
                <Label htmlFor="chestNumber">In số ngực</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="pantsNumber"
                  checked={player.pants_number || false}
                  onCheckedChange={checked => onPlayerChange({ pants_number: checked === true })}
                />
                <Label htmlFor="pantsNumber">In số quần</Label>
              </div>
            </div>
          </div>

          <div>
            <Label className="mb-2 inline-block">Logo áo</Label>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="logoChestLeft"
                  checked={player.logo_chest_left || false}
                  onCheckedChange={checked => onPlayerChange({ logo_chest_left: checked === true })}
                />
                <Label htmlFor="logoChestLeft">Logo ngực trái</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="logoChestRight"
                  checked={player.logo_chest_right || false}
                  onCheckedChange={checked => onPlayerChange({ logo_chest_right: checked === true })}
                />
                <Label htmlFor="logoChestRight">Logo ngực phải</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="logoChestCenter"
                  checked={player.logo_chest_center || false}
                  onCheckedChange={checked => onPlayerChange({ logo_chest_center: checked === true })}
                />
                <Label htmlFor="logoChestCenter">Logo ngực giữa</Label>
              </div>
            </div>
          </div>

          <div>
            <Label className="mb-2 inline-block">Logo tay & quần</Label>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="logoSleeveLeft"
                  checked={player.logo_sleeve_left || false}
                  onCheckedChange={checked => onPlayerChange({ logo_sleeve_left: checked === true })}
                />
                <Label htmlFor="logoSleeveLeft">Logo tay trái</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="logoSleeveRight"
                  checked={player.logo_sleeve_right || false}
                  onCheckedChange={checked => onPlayerChange({ logo_sleeve_right: checked === true })}
                />
                <Label htmlFor="logoSleeveRight">Logo tay phải</Label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="logoPants"
                  checked={player.logo_pants || false}
                  onCheckedChange={checked => onPlayerChange({ logo_pants: checked === true })}
                />
                <Label htmlFor="logoPants">Logo quần</Label>
              </div>
            </div>
          </div>
        </div>
        
        <div className="md:col-span-3">
          <Label htmlFor="playerNote">Ghi chú</Label>
          <Input
            id="playerNote"
            value={player.note || ""}
            onChange={e => onPlayerChange({ note: e.target.value })}
            placeholder="Ghi chú đặc biệt cho cầu thủ"
          />
        </div>
        
        <div className="md:col-span-2 flex space-x-2">
          {isEditing ? (
            <>
              <Button onClick={onSubmit} className="flex-1 bg-blue-600 hover:bg-blue-700">
                Cập nhật
              </Button>
              <Button variant="outline" onClick={onCancel} className="flex-1">
                Hủy
              </Button>
            </>
          ) : (
            <Button onClick={onSubmit} className="w-full">
              <Plus className="h-4 w-4 mr-1" /> Thêm cầu thủ
            </Button>
          )}
        </div>
      </div>
    </div>
  );
});

PlayerFormFields.displayName = "PlayerFormFields";

export default PlayerFormFields;
